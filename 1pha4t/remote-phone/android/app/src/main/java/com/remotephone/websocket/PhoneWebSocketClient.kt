...
class PhoneWebSocketClient(
    ...
    fun disconnect() {
        ...
        // разблокирует readFrame() в connect loop мгновенно
        try { socket?.close() } catch (_: Exception) {}
        // best-effort close frame через send queue
        try { sendExecutor.execute { try { sendFrame(0x08, ByteArray(0)) } catch (_: Exception) {} } } catch (_: Exception) {}
    }
